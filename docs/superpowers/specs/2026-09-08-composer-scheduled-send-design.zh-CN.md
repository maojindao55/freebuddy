...
面板刻意精简：一行标题、一行五个快捷延时、（可选）一条额度重置按钮、一行自定义时间，不放说明文字和分组标签。

1. **快捷延时**：15 分 / 30 分 / 1 小时 / 2 小时 / 5 小时，单行五等分
2. **额度重置时**：仅 Codex 系 adapter（`codex`、`codex-acp`）显示。面板打开时调用
   `cliClient.codexUsage()`，从各窗口里挑选目标重置时间（见「Codex 重置时间选择」），
   在重置时刻上再加 60 秒余量。读取中按钮禁用并显示「读取 Codex 额度…」；读取失败或无窗口时整条隐藏。
3. **自定义时间**：`datetime-local` 输入 + 「确定」；必须晚于当前时间。
