...
test("IPC exposes preview/transfer and resolves handoff briefs inside the main process", () => {
  ...
  assert.match(ipc, /buildTransferSeedPrompt/);
...
test("handoff UI auto-starts with a reference card and keeps per-conversation drafts isolated", () => {
  ...
  const contextDialog = read("src/components/CLI/ConversationContextDialog.tsx");
  ...
  assert.match(contextDialog, /ConversationContextDialog/);
  assert.match(contextDialog, /ShareConversationPanel/);
  assert.match(contextDialog, /TransferConversationPanel/);
  assert.match(contextDialog, /conversationContext\.shareOptionTitle/);
  assert.match(contextDialog, /conversationContext\.transferOptionTitle/);
  ...
  assert.match(chat, /ContextReferenceCard/);
  assert.match(chat, /context-reference-tray/);
  assert.match(chat, /context-reference-card/);
  assert.match(chat, /context-reference-card-avatar/);
  assert.match(chat, /AgentAvatar/);
  assert.match(chat, /sourceAdapter/);
  ...
  assert.doesNotMatch(chat, /context-reference-card[\s\S]*removeReference/);
  assert.doesNotMatch(
    chat,
    /composer-context-row[\s\S]*SharedConversationReferences[\s\S]*AttachmentTray/
  );
  ...
  assert.match(app, /contextSourceId/);
  assert.match(app, /ConversationContextDialog/);
  assert.doesNotMatch(app, /transferSourceId|shareSourceId/);
  assert.doesNotMatch(app, /titlebar-transfer-button/);
  assert.match(css, /\.titlebar-icon-button/);
  assert.match(css, /\.titlebar-actions\s*\{[\s\S]*?margin-left: auto/);
  assert.match(css, /\.titlebar\s*\{[\s\S]*?padding: 0 8px 0 16px/);
  assert.match(shareDialog, /createConversationShare/);
  assert.match(shareDialog, /ShareConversationPanel/);
  assert.match(css, /\.transfer-dialog\s*\{/);
  assert.match(css, /\.titlebar-transfer-button\s*\{/);
  assert.match(css, /\.conversation-context-option\s*\{/);
  assert.match(css, /\.context-reference-card\s*\{/);
  assert.match(css, /\.context-reference-card-avatar\s*\{/);
  assert.doesNotMatch(css, /\.handoff-reference-card\s*\{/);
  assert.doesNotMatch(css, /\.context-share-card\s*\{/);
